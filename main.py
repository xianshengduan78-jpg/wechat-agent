...
def cmd_daily(dry_run=False):
    """运行日报 pipeline（完整 11 步）。"""
    import logging
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")

    from pipelines.daily_pipeline import run_daily
    result = run_daily(dry_run=dry_run)

    if result:
        print(f"\n📋 日报 pipeline 完成")
        if "article" in result:
            print(f"   标题: {result['article'].get('article_title', '')}")
...
def cmd_article(topic=None, dry_run=False):
    ...
    from pipelines.article_pipeline import run_article
    result = run_article(topic, dry_run=dry_run)
